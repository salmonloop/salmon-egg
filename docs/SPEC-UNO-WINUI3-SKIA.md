# UnoAcpClient UI/运行时 SPEC（.NET 10 + Uno + WinUI3 + Skia）
...
### 1.1 项目框架
`C:\Users\shang\Project\salmon-acp\UnoAcpClient\UnoAcpClient\UnoAcpClient.csproj`
...
**MSIX 签名与信任**
- 使用自签名证书（Subject：`O=UnoAcpClient`）签名。
...
**守门测试（必须保持通过）**
- `C:\Users\shang\Project\salmon-acp\tests\UnoAcpClient.Application.Tests\UiConventionsTests.cs`
...
- `C:\Users\shang\Project\salmon-acp\UnoAcpClient\UnoAcpClient\DependencyInjection.cs`
...
`AccentBrush`（强调色）：
- 定义：`C:\Users\shang\Project\salmon-acp\UnoAcpClient\UnoAcpClient\App.xaml`
- 使用：所有页面强调色必须引用 `{ThemeResource AccentBrush}`，不直接引用 `SystemAccentColor`。

**守门测试（必须保持通过）**
- `C:\Users\shang\Project\salmon-acp\tests\UnoAcpClient.Application.Tests\UiConventionsTests.cs` 会扫描 XAML 禁止旧键。
...
实现位置：
- `C:\Users\shang\Project\salmon-acp\src\UnoAcpClient.Domain\Models\Protocol\SessionUpdateTypes.cs`
...
- `dotnet test tests\UnoAcpClient.Application.Tests\UnoAcpClient.Application.Tests.csproj` 通过
