...
- 云同步当前支持 OneDrive、WebDAV 与 S3-compatible object storage，设置页一次只能连接一个 provider；WebDAV 的文件夹 URL/用户名和 S3 的 endpoint/bucket/region/object key 属于用户配置，WebDAV 同步包默认文件名为 `salmonegg-config.zip`，WebDAV 密码与 S3 access key/secret key 走应用 secure storage；
