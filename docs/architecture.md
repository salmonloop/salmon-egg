...
- **配置云同步**：同步核心只依赖 provider-neutral 接口；当前支持 OneDrive（MSAL.NET + Graph，上传前确保 app folder 子目录存在）、WebDAV（用户配置远端文件夹 URL，应用使用默认文件名 `salmonegg-config.zip`，保存时按需创建缺失 collection）和 S3-compatible object storage（用户配置 endpoint/bucket/object key，object key 前缀不需要目录创建），UI 同一时间只能启用一个 provider。同步包包含 `config/` 和已登记的配置相关凭据，`secrets.json` 为明文内容。
