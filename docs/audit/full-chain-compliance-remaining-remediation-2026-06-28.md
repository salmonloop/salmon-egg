...
- The current ACP session modes documentation now defines `current_mode_update.currentModeId`. Do not treat `current_mode_update.modeId` as a standard field; `session/set_mode.modeId` and `modes.currentModeId` are separate standard fields scoped to their own ACP types.
