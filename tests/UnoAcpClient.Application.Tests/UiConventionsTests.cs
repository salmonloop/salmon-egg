...
namespace UnoAcpClient.Application.Tests;
...
public class UiConventionsTests
{
    private static string FindRepoRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(dir.FullName, "UnoAcpClient.sln")))
            {
                return dir.FullName;
            }
        ...
        throw new InvalidOperationException("Unable to locate repo root (UnoAcpClient.sln not found).");
    }
    ...
    public void XamlCodeBehind_ShouldResolveViewModelsBeforeInitializeComponent()
    {
        ...
        var uiRoot = Path.Combine(repoRoot, "UnoAcpClient", "UnoAcpClient");
    ...
    {
        var repoRoot = FindRepoRoot();
        var diFile = Path.Combine(repoRoot, "UnoAcpClient", "UnoAcpClient", "DependencyInjection.cs");
