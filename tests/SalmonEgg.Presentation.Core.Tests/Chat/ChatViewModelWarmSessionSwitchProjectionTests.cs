...
public partial class ChatViewModelTests
{
    ...
    [Fact]
    public async Task SwitchConversationAsync_WhenReturningToWarmRemoteConversationWithThinkingOnlyShell_MaterializesTranscriptFromRuntimeProjection()
    {
        // Evidence chain:
        // 1) SelectionOnly clear/resync can leave a thinking-only or ShowPlanPanel-only content shell.
        // 2) HasProjectedConversationContent treats that shell as projected and used to skip materialize.
        // 3) Warm short-circuit then skips session/load, so A->B->A projected a blank durable transcript.
        // 4) Materialize must compare durable transcript/plan against RuntimeProjection and restore it.
        var syncContext = new QueueingSynchronizationContext();
        var chatService = CreateConnectedChatService();
        chatService.SetupGet(service => service.AgentCapabilities).Returns(new AgentCapabilities(loadSession: true));
        chatService.Setup(service => service.LoadSessionAsync(
        ...
        var warmMessage = new ConversationMessageSnapshot
        {
            ...
            IsOutgoing = false,
            ContentType = "text",
            TextContent = "warm remote A durable transcript must return"
        };
        var thinkingShell = new ConversationMessageSnapshot
        {
            Id = "think-a-1",
            Timestamp = new DateTime(2026, 5, 2, 0, 0, 2, DateTimeKind.Utc),
            IsOutgoing = false,
            ContentType = "thinking",
            TextContent = "ephemeral thinking shell"
        };
        ...
        // Simulate hollow projected shell left after SelectionOnly clear / partial stream:
        // thinking-only content still "projects" but has no durable transcript.
        await fixture.UpdateStateAsync(state => state with
        {
            ConversationContents = (state.ConversationContents ?? ImmutableDictionary<string, ConversationContentSlice>.Empty)
                .SetItem(
                    "conv-a",
                    new ConversationContentSlice(
                        ImmutableList.Create(thinkingShell),
                        ImmutableList<ConversationPlanEntrySnapshot>.Empty,
                        false))
        });
