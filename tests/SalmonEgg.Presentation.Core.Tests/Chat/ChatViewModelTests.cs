...
public partial class ChatViewModelTests
{
        ...
        bool enableWorkspacePersistence = false)
    {
        ...
        var workspace = new ChatConversationWorkspace(
            ...
            uiDispatcher);
        var workspaceWriter = enableWorkspacePersistence
            ? new WorkspaceWriter(workspace, new ImmediateUiDispatcher())
            : null;
        var chatStore = new RecordingChatStore(state, workspaceWriter);
    ...
    private static Mock<ISessionManager> CreateSessionManagerWithStore()
    {
        ...
        // cwd 按契约就是非空的（会话没有工作目录这种状态不存在），这里如实照抄签名，
        // 免得测试替身比被替身更宽松、把产品代码不可能收到的 null 喂进来。
        sessionManager.Setup(s => s.CreateSessionAsync(It.IsAny<string>(), It.IsAny<string>()))
            .Returns<string, string>((sessionId, cwd) =>
            {
                var session = new Session(sessionId, cwd);
    ...
    public async Task ReplaceChatService_WhenCalledOffUiContext_DoesNotBlockCaller()
    {
        ...
        var replaceTask = Task.Run(() => fixture.ViewModel.ReplaceChatService(chatService.Object), TestContext.Current.CancellationToken);
        var completedTask = await Task.WhenAny(replaceTask, Task.Delay(250, TestContext.Current.CancellationToken));
    ...
    public async Task SelectProfile_WhenCalledOffUiContext_DoesNotBlockCaller()
    {
        ...
        var selectTask = Task.Run(() => fixture.ViewModel.SelectProfile(profile), TestContext.Current.CancellationToken);
        var completedTask = await Task.WhenAny(selectTask, Task.Delay(250, TestContext.Current.CancellationToken));
    ...
    public async Task ArchiveConversation_CurrentSession_ClearsTerminalPanelState()
    {
        ...
        var archiveResult = await fixture.ViewModel.ArchiveConversationAsync("session-1", TestContext.Current.CancellationToken);
    ...
    public async Task DeleteConversation_CurrentSession_ClearsTerminalPanelState()
    {
        ...
        var deleteResult = await fixture.ViewModel.DeleteConversationAsync("session-1", TestContext.Current.CancellationToken);
    ...
    public async Task ArchiveConversation_WhenCoordinatorReturnsFailedResult_PreservesTerminalPanelState()
    {
        ...
        await fixture.ViewModel.ArchiveConversationAsync("session-1", TestContext.Current.CancellationToken);
    ...
    public async Task ArchiveConversation_WhenCoordinatorIsSlow_DoesNotBlockCallerThread()
    {
        ...
        var archiveTask = fixture.ViewModel.ArchiveConversationAsync("session-1", TestContext.Current.CancellationToken);
        var completed = await Task.WhenAny(archiveTask, Task.Delay(200, TestContext.Current.CancellationToken));
    ...
    public async Task DeleteConversation_AfterRemoteSessionInfoUpdate_DoesNotResurrectDeletedConversation()
    {
        ...
        await fixture.Workspace.RegisterConversationAsync("session-1", cancellationToken: TestContext.Current.CancellationToken);
        ...
        await fixture.ViewModel.DeleteConversationAsync("session-1", TestContext.Current.CancellationToken);
    ...
    public async Task OpenDiscoveredRemoteSessionAsync_PersistsAuthoritativeCwdAndOrderedAdditionalDirectories()
    {
        ...
        var result = await switcher.OpenDiscoveredRemoteSessionAsync(
            new DiscoverRemoteSessionOpenRequest(
                ...
                sourceDirectories),
            TestContext.Current.CancellationToken);
        sourceDirectories[0] = @"C:\shared\mutated";

        Assert.True(result.Succeeded, result.ErrorMessage);
        Assert.False(string.IsNullOrWhiteSpace(result.LocalConversationId));
        var conversationId = result.LocalConversationId!;
        var snapshot = fixture.Workspace.GetConversationSnapshot(conversationId);
        var binding = fixture.Workspace.GetRemoteBinding(conversationId);

        Assert.NotNull(snapshot);
        Assert.Equal(@"C:\remote\workspace", snapshot.SessionInfo?.Cwd);
        Assert.Equal(
    ...
    public async Task AcpProfileList_AddForStoredSelection_DefersSelectedProfileProjectionUntilCollectionObserversCanSettle()
    {
        ...
        await AwaitWithSynchronizationContextAsync(
            syncContext,
            fixture.ViewModel.SelectProfileAsync(detachedProfile, TestContext.Current.CancellationToken));
    ...
    public async Task EnsureAcpProfilesLoadedAsync_WhenRefreshIsInFlight_WaitsForLoadedProfiles()
    {
        ...
        await ensureTask.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        await refreshTask.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
    ...
    public async Task CurrentAgentDisplayText_PrefersProfileName_ThenProtocolAgentName_ThenDash()
    {
        ...
        fixture.Profiles.Profiles.Add(profile);
        await fixture.ViewModel.SelectProfileAsync(profile, TestContext.Current.CancellationToken);
    ...
    public async Task SelectedAcpProfile_Change_PreservesActiveRemoteBindingUntilReconnectCompletes()
    {
        ...
        fixture.ViewModel.SelectedAcpProfile = profileB;
        await connectStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
        await WaitForConditionAsync(async () =>
        {
            var state = await fixture.GetStateAsync();
            return state.ResolveBinding("session-1") == new ConversationBindingSlice("session-1", "remote-1", "profile-a")
    ...
    public async Task Dispose_DoesNotDisposeInjectedConversationWorkspace()
    {
        ...
        try
        {
            ...
            var prepared = await workspace.TryPrepareConversationActivationAsync("session-1", TestContext.Current.CancellationToken);
    ...
    public async Task EnsureNewSessionDraftAsync_WhenAgentSupportsClose_CreatesDraftOnCurrentConnection()
    {
        ...
        await fixture.ViewModel.EnsureNewSessionDraftAsync(@"C:\Repo\App", TestContext.Current.CancellationToken);
    ...
    public async Task EnsureNewSessionDraftAsync_WhenReplacingDraftWithoutSessionCloseSupport_RemovesLocalDraftSession()
    {
        ...
        await fixture.ViewModel.EnsureNewSessionDraftAsync(@"C:\Repo\New", TestContext.Current.CancellationToken);
    ...
    public async Task EnsureNewSessionDraftAsync_WhenRequiredProfileDiffersFromForeground_ClosesStaleDraftWithoutReuse()
    {
        ...
        await fixture.ViewModel.EnsureNewSessionDraftForProfileAsync(
            @"C:\Repo\App",
            requiredProfileId: "profile-2", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task EnsureNewSessionDraftAsync_WhenSelectedIntentMovesAwayFromRequiredProfile_StopsWaitingForStaleDraft()
    {
        ...
        var completed = await Task.WhenAny(ensureTask, Task.Delay(TimeSpan.FromMilliseconds(750), TestContext.Current.CancellationToken));
    ...
    public async Task EnsureNewSessionDraftAsync_WhenRequiredProfileIntentIsNotForeground_QueuesProfileConnection()
    {
        ...
        await ownedFixture.ViewModel.ReplaceChatServiceAsync(CreateConnectedChatService().Object, TestContext.Current.CancellationToken);
        ...
        await ownedFixture.ViewModel.EnsureNewSessionDraftForProfileAsync(
            @"C:\Repo\App",
            requiredProfileId: "profile-remote", cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task EnsureNewSessionDraftAsync_WhenJoinedRequestStillDesired_AdoptsResponseAfterOriginalTokenCancels()
    {
        ...
        await createStarted.Task.WaitAsync(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken);
    ...
    public async Task PromoteNewSessionDraftForLaunchAsync_BindsReadyDraftWithoutCreatingAnotherRemoteSession()
    {
        ...
        await fixture.ViewModel.PromoteNewSessionDraftForLaunchAsync(TestContext.Current.CancellationToken);
    ...
    private sealed class ForwardingAcpConnectionCommands : IAcpConnectionCommands
    {
        ...
        public Task CancelPromptAsync(
            IAcpChatCoordinatorSink sink,
            CancellationToken cancellationToken = default)
            => RequireInner().CancelPromptAsync(sink, cancellationToken);
    ...
    private sealed class ViewModelFixture : IAsyncDisposable
    {
        ...
        public ValueTask MarkUnreadAsync(string conversationId, ConversationAttentionSource source)
            => _conversationAttentionStore.Dispatch(
                new MarkConversationUnreadAction(
                    conversationId,
                    source,
        ...
        public async Task FlushWorkspaceAsync()
        {
            if (_workspaceWriter is null)
            {
                return;
            }

            _workspaceWriter.Enqueue(_chatStore.LatestState, scheduleSave: true);
            var flushTask = _workspaceWriter.FlushAsync();
            if (_uiDispatcher is QueueingSynchronizationContext queuedDispatcher)
            {
                await queuedDispatcher.RunUntilCompletedAsync(flushTask);
                return;
            }
        ...
        {
            ViewModel.Dispose();
            if (_workspaceWriter is IDisposable disposableWorkspaceWriter)
            {
                disposableWorkspaceWriter.Dispose();
            }
    ...
    private sealed class RecordingChatStore : IChatStore
    {
        ...
        public RecordingChatStore(IState<ChatState> state, IWorkspaceWriter? workspaceWriter = null)
        {
            State = state;
            _workspaceWriter = workspaceWriter;
    ...
    public async Task CancelPromptCommand_WhenSubmitIsCancelledBeforeDispatch_DoesNotSendPrompt()
    {
        ...
        commands.Verify(x => x.DispatchPromptToRemoteSessionAsync(
            ...
            It.IsAny<Func<CancellationToken, Task<bool>>>(),
            It.IsAny<CancellationToken>()), Times.Never);
        commands.Verify(x => x.CancelPromptAsync(It.IsAny<IAcpChatCoordinatorSink>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task CancelPromptCommand_WhenPromptWasDispatched_WaitsForAuthoritativeCancelledResponseBeforeUnlockingNextPrompt()
    {
        ...
        await using var fixture = CreateViewModel(syncContext, acpConnectionCommands: commands.Object);
        var viewModel = fixture.ViewModel;
        await viewModel.ReplaceChatServiceAsync(CreateConnectedChatService().Object, TestContext.Current.CancellationToken);
        await fixture.UpdateStateAsync(state => state with
        {
            HydratedConversationId = "conv-1",
            Bindings = ImmutableDictionary<string, ConversationBindingSlice>.Empty
    ...
    public async Task CancelPromptCommand_PreemptivelyCancelsOutstandingToolCallsWithoutCompletingDispatchedTurn()
    {
        ...
        commands.Verify(x => x.CancelPromptAsync(It.IsAny<IAcpChatCoordinatorSink>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    [Fact]
    public async Task ProcessSessionUpdateAsync_SessionInfoUpdate_WithoutCwd_PreservesKnownCwd()
    {
        var syncContext = new ImmediateSynchronizationContext();
        var sessions = new Dictionary<string, Session>(StringComparer.Ordinal);
        var sessionManager = new Mock<ISessionManager>();
    ...
    public async Task RefreshMiniWindowSessions_ProjectsCompactDisplayNameWithoutChangingDisplayName()
    {
        ...
        sessionManager.Object.GetSession("conv-compact")!.DisplayName =
            "This session title should stay complete while the mini window trims it";
    ...
    public async Task ConversationWorkspaceChange_FromBackgroundThread_DefersConversationListProjectionToUiDispatcher()
    {
        ...
        await Task.Run(() =>
            fixture.Workspace.UpsertConversationSnapshot(new ConversationWorkspaceSnapshot(
                ...
                LastUpdatedAt: new DateTime(2026, 5, 13, 0, 0, 1, DateTimeKind.Utc))), TestContext.Current.CancellationToken);
    ...
    public async Task RefreshMiniWindowSessions_PreservesWholeGraphemeClustersWhenCompactDisplayNameTrims()
    {
        ...
        sessionManager.Object.GetSession("conv-emoji")!.DisplayName = fullDisplayName;
    ...
    public async Task ProcessSessionUpdateAsync_UsageUpdate_RetainsUsageInSessionStateWithoutUnhandledLog()
    {
        ...
        Assert.Equal(7UL, sessionState.Usage!.Used);
        Assert.Equal(128UL, sessionState.Usage.Size);
        Assert.NotNull(sessionState.Usage.Cost);
        Assert.Equal(1.5, sessionState.Usage.Cost!.Amount);
    ...
    public async Task ActiveWebSocketConversationTransportError_ReconnectsProfileAndHydratesRemoteSession()
    {
        ...
        fixture.Profiles.Profiles.Add(profile);
        await AwaitWithSynchronizationContextAsync(syncContext, fixture.ViewModel.ReplaceChatServiceAsync(staleService.Object, TestContext.Current.CancellationToken));
        await fixture.UpdateStateAsync(state => state with
        {
            HydratedConversationId = "conv-1",
            Bindings = ImmutableDictionary<string, ConversationBindingSlice>.Empty
    ...
    [Fact]
    public async Task HydrateActiveConversationAsync_WhenRemoteSessionListIsSlow_WaitsAndLoadsWithAuthoritativeCwd()
    {
        var syncContext = new ImmediateSynchronizationContext();
        var sessions = new Dictionary<string, Session>(StringComparer.Ordinal);
        var sessionManager = new Mock<ISessionManager>();
        ...
        var capturedParams = await loadStarted.Task.WaitAsync(
            TimeSpan.FromSeconds(30),
            TestContext.Current.CancellationToken);

        Assert.NotNull(capturedParams);
        Assert.Equal(@"C:\repo\fresh", capturedParams!.Cwd);
        Assert.Equal([@"C:\shared\first", @"D:\shared\second"], capturedParams.AdditionalDirectories);

        var projectedState = await fixture.GetStateAsync();
        var projectedSessionInfo = Assert.IsType<ConversationSessionInfoSnapshot>(
            projectedState.ResolveSessionStateSlice("conv-1")!.Value.SessionInfo);
        Assert.Equal("Remote title", projectedSessionInfo.Title);
        Assert.Equal(@"C:\repo\fresh", projectedSessionInfo.Cwd);
        Assert.Equal([@"C:\shared\first", @"D:\shared\second"], projectedSessionInfo.AdditionalDirectories);
        Assert.Equal(@"C:\repo\fresh", fixture.Workspace.GetConversationSnapshot("conv-1")!.SessionInfo!.Cwd);
        Assert.Equal(
            [@"C:\shared\first", @"D:\shared\second"],
            fixture.Workspace.GetConversationSnapshot("conv-1")!.SessionInfo!.AdditionalDirectories);
        Assert.Equal(@"C:\repo\fresh", sessions["conv-1"].Cwd);

        var hydrated = await hydrationTask;
        Assert.True(hydrated, fixture.ViewModel.ErrorMessage);
        chatService.Verify(
            service => service.ListSessionsAsync(It.IsAny<SessionListParams>(), It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    public async Task HydrateActiveConversationAsync_WhenSessionListCapabilityIsMissing_DoesNotCallSessionList()
    {
        ...
        chatService.Setup(service => service.LoadSessionAsync(It.IsAny<SessionLoadParams>(), It.IsAny<CancellationToken>()))
            .Callback<SessionLoadParams, CancellationToken>((value, _) => capturedLoadParams = value)
            .ReturnsAsync(SessionLoadResponse.Completed);
        chatService.Setup(service => service.ListSessionsAsync(It.IsAny<SessionListParams>(), It.IsAny<CancellationToken>()))
        ...
        fixture.Workspace.UpsertConversationSnapshot(new ConversationWorkspaceSnapshot(
            ...
            {
                Cwd = @"C:\repo\demo",
                AdditionalDirectories = [@"C:\shared\persisted"]
            }));
        fixture.Profiles.Profiles.Add(CreateConnectableStdioProfile("profile-1", "Profile 1"));
        ...
        Assert.True(hydrated, fixture.ViewModel.ErrorMessage);
        Assert.NotNull(capturedLoadParams);
        Assert.Equal([@"C:\shared\persisted"], capturedLoadParams!.AdditionalDirectories);
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task HydrateActiveConversationAsync_WhenSessionListCannotProvideMatch_UsesPersistedRecoveryContext(
        bool listThrows)
    {
        var syncContext = new ImmediateSynchronizationContext();
        var sessionManager = CreateSessionManagerWithStore();
        await sessionManager.Object.CreateSessionAsync("conv-1", @"C:\repo\persisted");
        SessionLoadParams? capturedLoadParams = null;
        var chatService = CreateConnectedChatService();
        chatService.SetupGet(service => service.AgentCapabilities)
            .Returns(new AgentCapabilities(
        ...
        chatService.Setup(service => service.ListSessionsAsync(It.IsAny<SessionListParams>(), It.IsAny<CancellationToken>()))
            .Returns<SessionListParams, CancellationToken>((_, _) => listThrows
                ? Task.FromException<SessionListResponse>(new InvalidOperationException("session/list failed"))
                : Task.FromResult(new SessionListResponse
                {
                    Sessions =
                    [
        ...
        fixture.Workspace.UpsertConversationSnapshot(new ConversationWorkspaceSnapshot(
            ...
            SessionInfo: new ConversationSessionInfoSnapshot
            {
                Cwd = @"C:\repo\persisted",
                AdditionalDirectories = [@"C:\shared\persisted", @"D:\shared\ordered"]
            }));
        await AwaitWithSynchronizationContextAsync(
            syncContext,
            fixture.ViewModel.ReplaceChatServiceAsync(chatService.Object, TestContext.Current.CancellationToken));
        ...
        Assert.True(hydrated, fixture.ViewModel.ErrorMessage);
        Assert.NotNull(capturedLoadParams);
        Assert.Equal(@"C:\repo\persisted", capturedLoadParams!.Cwd);
        Assert.Equal(
            [@"C:\shared\persisted", @"D:\shared\ordered"],
            capturedLoadParams.AdditionalDirectories);
        chatService.Verify(
            service => service.ListSessionsAsync(It.IsAny<SessionListParams>(), It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    [Fact]
    public async Task RestoreAndHydrateRemoteConversation_WhenSessionListReportsDifferentCwd_UpdatesProjectGrouping()
    {
        var syncContext = new ImmediateSynchronizationContext();
        var sessions = new Dictionary<string, Session>(StringComparer.Ordinal);
        var sessionManager = new Mock<ISessionManager>();
        ...
        await WaitForConditionAsync(
            () => Task.FromResult(
                string.Equals(fixture.Workspace.GetConversationSnapshot("conv-1")?.SessionInfo?.Cwd, @"C:\Users\shang\AppData\Local\SalmonEgg", StringComparison.Ordinal)
                && string.Equals(GetConversationCatalogPresenter(fixture.ViewModel).Snapshot.SingleOrDefault()?.Cwd, @"C:\Users\shang\AppData\Local\SalmonEgg", StringComparison.Ordinal)
                && string.Equals(sessionManager.Object.GetSession("conv-1")?.Cwd, @"C:\Users\shang\AppData\Local\SalmonEgg", StringComparison.Ordinal)),
            timeoutMilliseconds: 4000);
        navVm.RebuildTree();
        Assert.Equal(@"C:\Users\shang\AppData\Local\SalmonEgg", fixture.Workspace.GetConversationSnapshot("conv-1")!.SessionInfo!.Cwd);
        Assert.Equal(@"C:\Users\shang\AppData\Local\SalmonEgg", GetConversationCatalogPresenter(fixture.ViewModel).Snapshot.Single().Cwd);
        Assert.Equal(MainNavigationViewModel.UnclassifiedProjectId, navVm.TryGetProjectIdForSession("conv-1"));
    }

    [Fact]
    public async Task HydrateActiveConversationAsync_WhenWorkspaceSnapshotCarriesStaleCwd_UsesRemoteSessionListCwd()
    {
        var syncContext = new ImmediateSynchronizationContext();
        var sessions = new Dictionary<string, Session>(StringComparer.Ordinal);
        var sessionManager = new Mock<ISessionManager>();
        ...
        Assert.Equal(@"C:\Users\shang\AppData\Local\SalmonEgg", fixture.Workspace.GetConversationSnapshot("conv-1")!.SessionInfo!.Cwd);
        Assert.Equal(@"C:\Users\shang\AppData\Local\SalmonEgg", GetConversationCatalogPresenter(fixture.ViewModel).Snapshot.Single().Cwd);
        Assert.Equal(@"C:\Users\shang\AppData\Local\SalmonEgg", sessionManager.Object.GetSession("conv-1")!.Cwd);
    }

    [Fact]
    public async Task HydrateActiveConversationAsync_WhenRemoteSessionListReportsNewCwd_UsesRemoteCwd()
    {
        var syncContext = new ImmediateSynchronizationContext();
        var sessions = new Dictionary<string, Session>(StringComparer.Ordinal);
        var sessionManager = new Mock<ISessionManager>();
        ...
        await using (fixture)
        {
            ...
            fixture.Workspace.UpsertConversationSnapshot(new ConversationWorkspaceSnapshot(
                ...
                {
                    Cwd = @"C:\repo\stale",
                    AdditionalDirectories = [@"C:\shared\stale"]
                }));
            ...
            await fixture.ViewModel.HydrateActiveConversationAsync(TestContext.Current.CancellationToken);
            await WaitForConditionAsync(async () =>
            {
                var state = await fixture.GetStateAsync();
                return string.Equals(
            ...
            Assert.NotNull(capturedLoadParams);
            Assert.Null(capturedLoadParams!.AdditionalDirectories);
            Assert.Equal(@"C:\repo\fresh", storeSessionInfo.Cwd);
            Assert.NotNull(storeSessionInfo.AdditionalDirectories);
            Assert.Empty(storeSessionInfo.AdditionalDirectories);
            Assert.Equal(@"C:\repo\fresh", fixture.Workspace.GetConversationSnapshot("conv-1")!.SessionInfo!.Cwd);
            Assert.Empty(fixture.Workspace.GetConversationSnapshot("conv-1")!.SessionInfo!.AdditionalDirectories!);
        }
    }
            ...
            await fixture.UpdateStateAsync(state => state with
            {
                ...
                ConversationSessionStates = ImmutableDictionary<string, ConversationSessionStateSlice>.Empty
                    .Add("conv-1", new ConversationSessionStateSlice(
                        ...
                        null,
                        new ConversationUsageSnapshot(7, 128, new ConversationUsageCostSnapshot(1.5, "USD"))))
            });
            await DispatchConnectedAsync(fixture, "profile-1");
    ...
    public async Task ActivateConversationAsync_RemoteBoundConversation_LoadsRemoteSessionWhenConnected()
    {
        ...
        await using (fixture)
        {
            ...
            var activated = await fixture.ViewModel.SwitchConversationAsync("conv-2", TestContext.Current.CancellationToken);
    ...
    public async Task ActivateConversationAsync_WhileActivationStillRunning_ShowsOverlayBeforeHydrationStarts()
    {
        ...
        var activationTask = fixture.ViewModel.SwitchConversationAsync("conv-1", TestContext.Current.CancellationToken);
    ...
    public async Task SwitchConversationAsync_RemoteBoundConversation_WhenProfileConnectIsPending_KeepsOverlayVisible()
    {
        ...
        var switchTask = fixture.ViewModel.SwitchConversationAsync("conv-2", TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task SwitchConversationAsync_RemoteActivationPreConnect_WhenProfileConnectIsPending_ProjectsConnectionOverlay()
    {
        var syncContext = new QueueingSynchronizationContext();
        var runtimeState = new ShellNavigationRuntimeStateStore
        {
        ...
        commands.Setup(x => x.ConnectToProfileAsync(
                ...
                It.Is<AcpConnectionContext>(context =>
                    ...
                    && context.ActivationVersion.HasValue),
                It.IsAny<CancellationToken>()))
            .Returns<ServerConfiguration, IAcpTransportConfiguration, IAcpChatCoordinatorSink, AcpConnectionContext, CancellationToken>(async (_, _, sink, _, cancellationToken) =>
            {
        ...
        var profile = new ServerConfiguration { Id = "profile-1", Name = "Profile 1", Transport = TransportType.Stdio };
        var configurationService = new Mock<IConfigurationService>();
        configurationService.Setup(service => service.LoadConfigurationAsync("profile-1"))
        ...
        {
            syncContext.RunAll();
            return Task.FromResult(connectEntered.Task.IsCompleted || switchTask.IsCompleted);
        });
        Assert.False(switchTask.IsCompleted);
        await WaitForConditionAsync(() =>
        {
            syncContext.RunAll();
            return Task.FromResult(fixture.ViewModel.IsConnecting);
        });

        Assert.True(fixture.ViewModel.IsConnecting);
    ...
    public async Task ShellOverlayProjection_WhenChatViewModelRaisesOverlayPropertyOffUiThread_MarshalsProjectionNotificationsToUiDispatcher()
    {
        ...
        syncContext.RunAll();
        await notificationCompleted.Task.WaitAsync(TimeSpan.FromSeconds(5), TestContext.Current.CancellationToken);
    ...
    public async Task Overlay_WhenVisible_DisablesPromptInputUntilActivationSettles()
    {
        ...
        string[] raisedSnapshot;
        lock (raisedSync)
        {
            raisedSnapshot = raised.ToArray();
        }

        Assert.Contains(nameof(ChatViewModel.CanSendPromptUi), raisedSnapshot);
        Assert.Contains(nameof(ChatViewModel.IsInputEnabled), raisedSnapshot);
    }
    ...
    public async Task ConversationSessionSwitcherContract_RemoteBoundConversation_DoesNotRegressOverlayStatusBackToPreparingAfterHydrationStarts()
    {
        ...
        var switcher = (IConversationSessionSwitcher)fixture.ViewModel;
        var activationTask = switcher.SwitchConversationAsync("conv-2", TestContext.Current.CancellationToken);
    ...
    public async Task ActivateConversationAsync_RemoteBoundConversation_WhenCurrentSessionMatchesAndRuntimeIsWarm_SkipsReload()
    {
        ...
        var activated = await fixture.ViewModel.SwitchConversationAsync("conv-1", TestContext.Current.CancellationToken);
    ...
    public async Task ActivateConversationAsync_RemoteBoundConversation_WhenRuntimeIsWarmButSnapshotWasOnlyRestored_StillLoadsRemoteSession()
    {
        ...
        var switchedLocal = await fixture.ViewModel.SwitchConversationAsync("conv-local", TestContext.Current.CancellationToken);
        Assert.True(switchedLocal);

        var activated = await fixture.ViewModel.SwitchConversationAsync("conv-remote", TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task ConversationSessionSwitcherContract_WhenSupersededForegroundHydrationCompletesFromReusedLease_ReplayIsPreserved()
    {
        var syncContext = new QueueingSynchronizationContext();
        var sessions = new Dictionary<string, Session>(StringComparer.Ordinal);
        var sessionManager = new Mock<ISessionManager>();
        ...
        Assert.True(await localSwitchTask);
        syncContext.RunAll();
        var secondRemoteSwitchTask = fixture.ViewModel.SwitchConversationAsync("conv-remote", TestContext.Current.CancellationToken);
        await WaitForConditionAsync(() =>
        {
            syncContext.RunAll();
            return Task.FromResult(
        ...
        {
            syncContext.RunAll();
            return Task.FromResult(firstRemoteSwitchTask.IsCompleted);
        }, timeoutMilliseconds: 2000);
    ...
    public async Task ConversationSessionSwitcherContract_WhenSameLeaseStartsConcurrently_CreatesSingleLoad()
    {
        ...
        var firstLoadTask = fixture.ViewModel.HydrateActiveConversationAsync(TestContext.Current.CancellationToken);
        var secondLoadTask = fixture.ViewModel.HydrateActiveConversationAsync(TestContext.Current.CancellationToken);
        await WaitForConditionAsync(() =>
        {
            syncContext.RunAll();
    ...
    public async Task ConversationSessionSwitcherContract_WhenConflictingLeaseIsCanceled_StartsReplacementLoad()
    {
        ...
        var firstHydrateTask = fixture.ViewModel.HydrateActiveConversationAsync(TestContext.Current.CancellationToken);
        await WaitForConditionAsync(() =>
        {
            syncContext.RunAll();
            return Task.FromResult(firstLoadStarted.Task.IsCompleted);
        ...
        sessionManager.Object.GetSession("conv-remote")!.AdoptAuthoritativeCwd(@"C:\repo\new");
    ...
    public async Task ConversationSessionSwitcherContract_WhenHydrationAttemptIsSuperseded_DoesNotPublishStaleLoadAsWarm()
    {
        ...
        var hydrateTask = fixture.ViewModel.HydrateActiveConversationAsync(TestContext.Current.CancellationToken);
        await WaitForConditionAsync(() =>
        {
            syncContext.RunAll();
            return Task.FromResult(loadStarted.Task.IsCompleted);
    ...
    [Fact]
    public async Task ConversationSessionSwitcherContract_WhenBackgroundLoadIsSuperseded_StillWritesWorkspaceSnapshot()
    {
        var syncContext = new QueueingSynchronizationContext();
        var sessionManager = CreateSessionManagerWithStore();
        ...
        var loadStarted = new TaskCompletionSource<object?>(TaskCreationOptions.RunContinuationsAsynchronously);
        var allowLoadCompletion = new TaskCompletionSource<object?>(TaskCreationOptions.RunContinuationsAsynchronously);
        var loadCanceled = new TaskCompletionSource<object?>(TaskCreationOptions.RunContinuationsAsynchronously);
        var loadCompleted = new TaskCompletionSource<object?>(TaskCreationOptions.RunContinuationsAsynchronously);
        ...
        innerChatService = new ReplayLoadChatService
        {
            ...
            OnLoadSessionAsync = async (parameters, cancellationToken) =>
            {
                ...
                {
                    loadCanceled.TrySetResult(null);
                    throw;
                }

                innerChatService!.RaiseSessionUpdate(new SessionUpdateEventArgs(
                    "remote-1",
                    new AgentMessageUpdate(new TextContentBlock("superseded background replay"))));
                loadCompleted.TrySetResult(null);
                return SessionLoadResponse.Completed;
            }
        };
        ...
        await using var fixture = CreateViewModel(
            syncContext,
            sessionManager: sessionManager,
            enableWorkspacePersistence: true);
        await syncContext.RunUntilCompletedAsync(fixture.ViewModel.RestoreAsync(TestContext.Current.CancellationToken));
        await AwaitWithSynchronizationContextAsync(syncContext, fixture.ViewModel.ReplaceChatServiceAsync(adapter, TestContext.Current.CancellationToken));
        await fixture.UpdateStateAsync(state => state with
        {
        ...
        await syncContext.RunUntilCompletedAsync(remoteSwitchTask);
        Assert.True(await remoteSwitchTask);

        var localSwitchTask = switcher.SwitchConversationAsync("conv-local", CancellationToken.None);
        await syncContext.RunUntilCompletedAsync(localSwitchTask);
        Assert.True(await localSwitchTask);
        ...
        {
            syncContext.RunAll();
            return Task.FromResult(loadCanceled.Task.IsCompleted || loadCompleted.Task.IsCompleted);
        }, timeoutMilliseconds: 2000);

        Assert.False(loadCanceled.Task.IsCompleted);
        await WaitForConditionAsync(() =>
        {
            syncContext.RunAll();
            return Task.FromResult(fixture.ChatStore.LatestState.ResolveContentSlice("conv-remote")?.Transcript.Any(
                message => string.Equals(
                    message.TextContent,
                    "superseded background replay",
        ...
        {
            syncContext.RunAll();
            var snapshot = fixture.Workspace.GetConversationSnapshot("conv-remote");
            return Task.FromResult(snapshot?.Transcript.Any(
                message => string.Equals(
                    message.TextContent,
                    "superseded background replay",
                    StringComparison.Ordinal)) == true);
        ...
        var finalState = await fixture.GetStateAsync();
        Assert.Equal("conv-local", finalState.HydratedConversationId);
        Assert.Equal("conv-local", fixture.Workspace.LastActiveConversationId);
    }

    [Fact]
    public async Task ConversationSessionSwitcherContract_WhenSupersededBackgroundLoadCompletes_DoesNotClearUnreadAttention()
    {
        var syncContext = new QueueingSynchronizationContext();
        var sessionManager = CreateSessionManagerWithStore();
        ...
        innerChatService = new ReplayLoadChatService
        {
            ...
            OnLoadSessionAsync = async (parameters, cancellationToken) =>
            {
                ...
                innerChatService!.RaiseSessionUpdate(new SessionUpdateEventArgs(
                    "remote-1",
                    new AgentMessageUpdate(new TextContentBlock("superseded unread replay"))));
                loadCompleted.TrySetResult(null);
                return SessionLoadResponse.Completed;
            }
        };
        ...
        await syncContext.RunUntilCompletedAsync(localSwitchTask);
        Assert.True(await localSwitchTask);
        await fixture.MarkUnreadAsync("conv-remote", ConversationAttentionSource.AgentMessage);
        Assert.True(HasUnreadAttention(await fixture.GetAttentionStateAsync(), "conv-remote"));
        ...
        {
            syncContext.RunAll();
            return Task.FromResult(HasStructuredLog(
                fixture.ViewModelLogger,
                LogLevel.Information,
                "Conversation remote activation completed.",
                ("ConversationId", "conv-remote"),
                ("Hydrated", true),
                ("ActivationStillCurrent", false)));
        }, timeoutMilliseconds: 2000);

        var finalState = await fixture.GetStateAsync();
        var finalAttentionState = await fixture.GetAttentionStateAsync();
        Assert.Equal("conv-local", finalState.HydratedConversationId);
        Assert.True(HasUnreadAttention(finalAttentionState, "conv-remote"));
    }

    [Fact]
    public async Task ConversationSessionSwitcherContract_WhenSupersededBeforeLoadStarts_StillStartsLoadAndWritesWorkspaceSnapshot()
    {
        var syncContext = new QueueingSynchronizationContext();
        var sessionManager = CreateSessionManagerWithStore();
        ...
        {
            sessionListStarted.TrySetResult(null);
            await allowSessionListCompletion.Task.WaitAsync(cancellationToken);
            return new SessionListResponse
            {
                Sessions =
                [
        ...
        innerChatService.OnLoadSessionAsync = (parameters, _) =>
        {
            Assert.Equal("remote-1", parameters.SessionId);
            loadStarted.TrySetResult(null);
            innerChatService.RaiseSessionUpdate(new SessionUpdateEventArgs(
                "remote-1",
                new AgentMessageUpdate(new TextContentBlock("superseded before load replay"))));
            return Task.FromResult(SessionLoadResponse.Completed);
        };
        AcpChatServiceAdapter? adapter = null;
        var eventAdapter = new AcpEventAdapter(
        ...
        {
            syncContext.RunAll();
            return Task.FromResult(sessionListStarted.Task.IsCompleted);
        }, timeoutMilliseconds: 2000);
        ...
        allowSessionListCompletion.TrySetResult(null);
        await WaitForConditionAsync(() =>
        {
            syncContext.RunAll();
            return Task.FromResult(loadStarted.Task.IsCompleted);
    ...
    public async Task ConversationSessionSwitcherContract_WhenRecoveryLeaseIsCanceled_LateReplayIsDiscarded()
    {
        ...
        await AwaitWithSynchronizationContextAsync(syncContext, fixture.ViewModel.ReplaceChatServiceAsync(oldAdapter, TestContext.Current.CancellationToken));
        await fixture.UpdateStateAsync(state => state with
        {
            HydratedConversationId = "conv-local",
            Bindings = ImmutableDictionary<string, ConversationBindingSlice>.Empty
        ...
        var oldSwitchTask = fixture.ViewModel.SwitchConversationAsync("conv-remote", TestContext.Current.CancellationToken);
        await WaitForConditionAsync(() =>
        {
            syncContext.RunAll();
        ...
        await AwaitWithSynchronizationContextAsync(syncContext, fixture.ViewModel.ReplaceChatServiceAsync(newAdapter, TestContext.Current.CancellationToken));
        await fixture.DispatchConnectionAsync(new SetConnectionInstanceIdAction("conn-new"));
        var newSwitchTask = fixture.ViewModel.SwitchConversationAsync("conv-remote", TestContext.Current.CancellationToken);
        await WaitForConditionAsync(() =>
        {
            syncContext.RunAll();
    ...
    [Fact]
    public async Task ConversationSessionSwitcherContract_WhenSupersededOwnedLoadCompletes_ReleasesReplayScopeForOtherSessionUpdates()
    {
        var syncContext = new QueueingSynchronizationContext();
        var sessionManager = CreateSessionManagerWithStore();
    ...
    public async Task ConversationSessionSwitcherContract_WhenSecondRemoteSelectionSupersedesFirst_OnlyLatestRemoteReplayCanAffectVisibleTranscript()
    {
        ...
        var firstRemoteSwitchTask = fixture.ViewModel.SwitchConversationAsync("conv-remote-1", TestContext.Current.CancellationToken);
        await WaitForConditionAsync(() =>
        {
            syncContext.RunAll();
        ...
        var secondRemoteSwitchTask = fixture.ViewModel.SwitchConversationAsync("conv-remote-2", TestContext.Current.CancellationToken);
        await syncContext.RunUntilCompletedAsync(secondRemoteSwitchTask);
        ...
        {
            syncContext.RunAll();
            var staleSnapshot = fixture.Workspace.GetConversationSnapshot("conv-remote-1");
            return Task.FromResult(staleSnapshot?.Transcript.Any(
                message => message.TextContent?.Contains("stale remote-1 replay", StringComparison.Ordinal) == true) == true);
        }, timeoutMilliseconds: 2000);
    }
    ...
    public async Task SwitchConversationAsync_WhenStaleRemoteConnectCompletesLate_DoesNotReplaceCurrentIntentChatService()
    {
        ...
        await using (fixture)
        {
            ...
            var staleSwitchTask = fixture.ViewModel.SwitchConversationAsync("conv-remote-1", TestContext.Current.CancellationToken);
            await WaitForConditionAsync(() =>
            {
                syncContext.RunAll();
            ...
            var returnToLocalTask = fixture.ViewModel.SwitchConversationAsync("conv-local", TestContext.Current.CancellationToken);
