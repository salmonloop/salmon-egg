...
5. 当前沉淀的通用经验规则：
   ...
   - 触发条件：聊天消息进入可靠文本选择模式，且 Markdown 正文包含显式链接、自动链接、代码块或其它可交互 inline 内容；原生期望行为：继续使用 Markdown 控件的原生文本选择能力，并通过控件官方属性关闭链接解析/交互，使选择语义与链接语义互斥；禁止做法：用 pointer 事件、hit-test、手动恢复 selection、替换控件模板、或在 ViewModel 中感知鼠标状态来补偿 native 选择行为；验证方式：合同测试断言 Windows 可选择 Markdown 变体使用 `DisableLinks=true` 与 `UseAutoLinks=false`，且源码不引入 `PointerEntered`/`PointerMoved` 等拦截式修复，并用真实 GUI 路径覆盖跨链接拖选不丢失 selection、不打开链接、不触发 native crash。
